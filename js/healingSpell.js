...
async function hunt(message, client, id, username) { 
    ...
    db.query(query, async (err, result) => {
        ...
        if (cHp <= 0) {
            cHp = 0;
            if (stat.level > 1) {
                db.query(`UPDATE stat SET hp=1, mp=${maxMp}, level=level - 1, current_experience=0 WHERE player_id="${id}"`);
        ...
        if (totalXP >= stat.experience) {
            ...
            db.query(`SELECT id, experience FROM level WHERE experience<=${totalXP} ORDER BY id DESC LIMIT 1`, async (err, result) => {
                ...
                db.query(`UPDATE stat SET level="${result[0].id}", current_experience=${cExp}, hp="${maxHp}", mp="${maxMp}" WHERE player_id="${id}"`);
